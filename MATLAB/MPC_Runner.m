...
ref_0 = load('MPC_SETPOINT_1.mat'); 
ref_1 = load('MPC_SETPOINT_2.mat'); 
...
%buildInstrumentedMex MPC_Simulator -args {ref_0,ref_1,noise,T,H,q_,E,F1,z_min,z_max,n,m,N_h,A_d,B_d,C_d,Aob,Bob,Cob,Ld} -histogram

[y_t,u_t] = MPC_Simulator(ref_0,ref_1,T,H,q_,E,F1,z_min,z_max,n,m,N_h,A_d,B_d,C_d,Aob,Bob,Cob,Ld); 

%showInstrumentationResults MPC_Simulator ...
  %-defaultDT numerictype(1,16) -proposeFL


%codegen MPC_Simulator -args {ref_0,ref_1,noise,T,H,q_,E,F1,z_min,z_max,n,m,N_h,A_d,B_d,C_d,Aob,Bob,Cob,Ld} -config:lib -report

% Output Plotting

y_t_V = y_t / 7.15; % Scale by 7.15 to get the original sensor voltage

t = linspace(1,1400,1400)./10;
